...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  })}</tbody></table> <div class="join flex justify-center mt-4"><button class="join-item btn" ${"disabled"}>«</button> <button class="join-item btn">Page ${escape(currentPage)}</button> <button class="join-item btn" ${currentPage * itemsPerPage >= allTransactions.length ? "disabled" : ""}>»</button></div>` : `${$authStore.userType === "merchant" ? `<div class="w-full flex justify-between p-4" data-svelte-h="svelte-1i225wp"><h1 class="text-[28px] font-bold text-white">Recent Transactions</h1></div> <div class="w-full flex justify-between p-4" data-svelte-h="svelte-1wsdbhs"><p>No Transactions found</p></div>` : ``}`}</div>` : `<div class="w-full flex justify-between p-4" data-svelte-h="svelte-e3ko25"><h1 class="text-white font-bold text-[26px]">No campaigns yet.</h1></div> ${userType === "influencer" ? `<div class="w-full flex justify-between p-4" data-svelte-h="svelte-11dkodd"><p>Get in contact with a merchant for a partnership!</p></div>` : ``}`}</div>`}`;
});
export {
  Page as default
};
